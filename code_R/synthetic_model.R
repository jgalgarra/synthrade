...
SynthMatrix <- function(matrixemp, year){
  ...
  while ((morenewnodes) || (cuenta_links < numlinks))
  {
    ...
    if (nrow(update_links)>0){
      prop_links <- rep(0.0,nrow(update_links))
      sum_links <- 0
      for (k in 1:nrow(update_links))
        sum_links <- sum_links + Pr_E[update_links[k,1]]*Pr_I[update_links[k,2]]
      for (k in 1:nrow(update_links))
        prop_links[k] <- Pr_E[update_links[k,1]]*Pr_I[update_links[k,2]]/sum_links
    }
